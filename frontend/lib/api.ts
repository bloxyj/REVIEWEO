...
export function getArtistGenres(id: number): Promise<ArtistGenre[]> {
    return request<ArtistGenre[]>(`/artists/${id}/genres`);
}

export function getArtistRelated(id: number): Promise<ArtistRelated[]> {
    return request<ArtistRelated[]>(`/artists/${id}/related`);
}
...
export function listAlbums(params?: ApiListParams): Promise<Album[]> {
    return request<Album[]>(`/albums${toQuery(params)}`);
}
...
export function getAlbumTracks(id: number): Promise<AlbumTrack[]> {
    return request<AlbumTrack[]>(`/albums/${id}/tracks`);
}
...
export function listReviews(params?: ApiListParams): Promise<Review[]> {
    return request<Review[]>(`/reviews${toQuery(params)}`);
}
...
export function createAlbumReview(token: string, albumId: number, input: Omit<ReviewInput, 'album_id'>): Promise<Review> {
    return request<Review>(`/albums/${albumId}/reviews`, {
        method: 'POST',
        token,
        body: input,
    });
...
export function toggleReviewLike(token: string, reviewId: number): Promise<{ review_id: number; liked: boolean; total_likes: number }> {
    return request<{ review_id: number; liked: boolean; total_likes: number }>(`/likes/${reviewId}`, {
        method: 'POST',
        token,
    });
}
...
export function adminPinReview(token: string, reviewId: number, isPinned: boolean): Promise<Review> {
    return request<Review>(`/admin/pin/${reviewId}`, {
        method: 'POST',
        token,
